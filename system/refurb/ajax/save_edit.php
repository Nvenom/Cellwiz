...
$UPDATE = MYSQL::QUERY('UPDATE core_refurb_devices SET d_locked_price = ?, d_unlocked_price = ? WHERE d_id = ? LIMIT 1',ARRAY($l,$u,$r));
$REFURB = MYSQL::QUERY('SELECT * FROM core_refurb_devices WHERE d_id = ? LIMIT 1',ARRAY($r));
$Model = MYSQL::QUERY("SELECT * FROM device_models WHERE m_id = ? LIMIT 1", ARRAY($REFURB['d_model_id']));
...
<center>
    <img src='https://my-cpr.com/barcode.php?encode=I25&height=20&scale=1&color=000000&bgcolor=FFFFFF&type=png&file=&bdata=".$r."' border='0' /><br/>
	Refurb#: ".$r."<br/>
	Identifier<br/>".$REFURB['d_iden']."<br/>
	...
	<div style='position:absolute;bottom:25px;right:5px;width:50%;'>
	    ...
	    <b>Carrier:</b> ".$REFURB['d_service_provider']."<br/>
	    <b>Price:</b> $".$REFURB['d_locked_price']."<br/>
	    ";
	        if($REFURB['d_unlocked_price'] > $REFURB['d_locked_price']){
		        echo "<b>UnLocked Price:</b> $".$REFURB['d_unlocked_price']."<br/><br/>";
		    } else {
		        echo "<br/><br/>";
		    }
	    echo "
