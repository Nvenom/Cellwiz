<?php 
	...
	MYSQL::QUERY("INSERT INTO core_tickets_sale (t_customer, t_items, t_cost, t_date) VALUES (?,?,?,?)", ARRAY($cus, $items, $totalcost, $date));
	MYSQL::QUERY("INSERT INTO core_checkout_sessions (customer, items, pm_1, pm_1_cost, pm_2, pm_2_cost, ses, s_id, d_date) VALUES (?,?,?,?,?,?,?,?,?)", ARRAY($cus, $items, $pm1, $pm1cost, $pm2, $pm2cost, $ses, $user['store'], Date("Y-m-d H:i:s")));
	$CUSTOMER = MYSQL::QUERY("SELECT * FROM core_customers WHERE c_id = ? LIMIT 1", ARRAY($cus));
    echo "
        <style>
            .escca td{
                border-top: 0px;
	        ...
	        <table border='1' cellspacing='0' id='Customer-Table' class='escca' style='border: 1px solid black;width: 98% !important;border-left: 0px;border-bottom:0px none !important;border-top:0px none;'>
	            <tbody>
			        ...
			        <tr style='font-family:Courier, monospace;'>
				        ...
				        <td>".FORMAT::PHONE($CUSTOMER['c_phone'])."</td>
				        <td>".Date('m/d/y')."</td>
				        <td>".Date('h:i A')."</td>
			        </tr>
		        </tbody>
