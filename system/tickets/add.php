...
function GetCustomersC(event, string)
{
    if(string.length > 3 || event.keyCode==13){
        $("#customerid").attr('disabled', true).trigger("liszt:updated");
        $.ajax({
            url: "tickets/ajax/customerid_template_2.php?string=" + string,
            cache: false
        }).done(function( html ) {
            $("#customerid").attr("disabled", false).html(html).trigger("liszt:updated");
        });
...
<div class="centerfloat" style="width:100%;height:100%;overflow-x:hidden;overflow-y:auto;">
    <div style="width:100%; height:100%;overflow: auto;">
	    <h3 class="block-banner">Tickets - Repair Ticket</h3>
		<div style="width:98%;background-color:lightgrey;margin-bottom:14px;" class="final-container">
		    <div style="width: 98%; height: 100%;padding-top:10px;padding-bottom:10px;">
                <form id="ticket_data" style="display: block;" method="get">
					...
					<input type="text" id="custsearch" class="ignore" title="Scan or Typer My-CPR Card# to load Customer" placeholder="My-CPR Card#" onKeyUp="GetCustomersC(event, $(this).val())">
					<br/>
                        <table id="form_template">
                        </table>
					<input type="hidden" id="device_amount" name="device_amount" value="0">
				</form>
				<button type="button" id="remmult" onClick="decreasePage($('#Runamnt').val() - 0);" title="Decrease">-</button><input type="text" style="width:30px;text-align:center;" maxlength="2" id='Runamnt' value="1" title="Increase & Decrease Amount"><button type="button" id="addmult" onClick="increasePage($('#Runamnt').val() - 0);" title="Increase">+</button>&nbsp;<button id="unlockform" onClick="UnlockForm()">Print Unlock Form</button>&nbsp;<button id="createt" onClick="CreateTickets()">Create Ticket/s</button>
			</div>
        </div>
    </div>
</div>
